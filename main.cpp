...
void Test6() {
    ...
    }
    {
        ...
        auto g = v.Insert(v.cbegin() + 2, v[0]);
        std::vector<bool> vec;
        std::all_of(v.begin(), v.end(), [&vec](const TestObj& obj) {
            vec.push_back(obj.IsAlive());
            return obj.IsAlive();
        });
    ...
    }
    {
        ...
        std::vector<bool> vec;
        std::for_each(v.begin(), v.end(), [&vec](const TestObj& obj) {
            vec.push_back(obj.IsAlive());
            return obj.IsAlive();
        });
        assert(std::all_of(v.begin(), v.end(), [](const TestObj& obj) {
    ...
    }
    {
        ...
        Vector<Obj> v{SIZE};
        int i =0;
        std::for_each(v.begin(), v.end(), [&i](Obj& obj){
           obj.id = i;
           i++;
        });
        v[2].id = ID;
        std::vector<int> ids_before;
        std::for_each(v.begin(), v.end(), [&ids_before](const Obj& obj){
            ids_before.push_back(obj.id);
        });
        std::vector<int> ids;

        auto* pos = v.Erase(v.cbegin() + 1);
        std::for_each(v.begin(), v.end(), [&ids](const Obj& obj){
